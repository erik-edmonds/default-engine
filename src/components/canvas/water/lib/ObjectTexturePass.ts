...
// Ported from water/src/rendering/ObjectTexturePass.ts, near-verbatim
// (framework-agnostic Three.js class, no React coupling). Generates
// reflection/clippedReflection/refraction/shadow render targets for
// whichever "mesh-kind" object is currently active (Scuba here), consumed
// by the WaterAbove/WaterBelow/Caustics shader materials.
export class ObjectTexturePass {
  ...
  setSize(width: number, height: number) {
    ...
    this.clippedReflectionTarget.setSize(Math.max(1, Math.floor(width * scale)), Math.max(1, Math.floor(height * scale)))
  ...
  private renderRefraction(scene: THREE.Scene, camera: THREE.PerspectiveCamera, materials: THREE.ShaderMaterial[]) {
  ...
  private renderReflection(scene: THREE.Scene, camera: THREE.PerspectiveCamera, materials: THREE.ShaderMaterial[]) {
  ...
  private withOnlyObjectVisible(scene: THREE.Scene, renderableObject: THREE.Object3D, render: () => void) {
  ...
  dispose() {
    ...
    this.clippedReflectionTarget.dispose()
