...
vec3 getWallColor(vec3 point) {
  ...
  // Ported constant assumed a ~1-unit pool (walls ~1-2 units away); this
  // pool is roughly 8-13 units across, so a bare 1/distance falloff left
  // everything past the first couple of units nearly black. Normalizing by
  // a reference distance keeps comparable brightness at this pool's actual
  // scale instead of just getting darker as the pool grows.
  scale /= max(1.0, length(point) / 8.0);
  if (sphereEnabled) {
  ...
  // Two lights, each with its own refraction/diffuse/caustics, added: a
  // wall facing away from light (previously just black, since diffuse
  // would be zero regardless of what the caustics texture held there) now
  // still picks up illumination from light2's own independent caustics
  // texture instead. Each light's causticTex is sampled with *that same
  // light's* own parallax offset -- never cross-sampling texture 1 with
  // light 2's offset or vice versa.
  vec3 refractedLight = -refract(-light, vec3(0.0, 1.0, 0.0), IOR_AIR / IOR_WATER);
  vec3 refractedLight2 = -refract(-light2, vec3(0.0, 1.0, 0.0), IOR_AIR / IOR_WATER);
  float diffuse1 = max(0.0, dot(refractedLight, normal));
  float diffuse2 = max(0.0, dot(refractedLight2, normal));

  vec4 info = texture2D(water, point.xz * vec2(0.5 / poolWidth, 0.5 / poolLength) + 0.5);

  if (point.y < info.r) {
    vec4 causticA = texture2D(
      causticTex,
      0.75 *
    ...
    vec4 causticB = texture2D(
      causticTex2,
      0.75 *
        (point.xz - point.y * refractedLight2.xz / refractedLight2.y) *
        vec2(0.5 / poolWidth, 0.5 / poolLength) +
        0.5
    );
    scale += diffuse1 * causticA.r * 3.5 * causticA.g + diffuse2 * causticB.r * 3.5 * causticB.g;
  } else {
    vec2 t1 = intersectRoundedBox(point, refractedLight, cornerRadius);
    float shadowed1 =
      diffuse1 /
      (1.0 +
        exp(-200.0 / (1.0 + 10.0 * (t1.y - t1.x)) * (point.y + refractedLight.y * t1.y - 2.0 / 12.0)));
    vec2 t2 = intersectRoundedBox(point, refractedLight2, cornerRadius);
    float shadowed2 =
      diffuse2 /
      (1.0 +
        exp(-200.0 / (1.0 + 10.0 * (t2.y - t2.x)) * (point.y + refractedLight2.y * t2.y - 2.0 / 12.0)));
    scale += (shadowed1 + shadowed2) * 0.5;
  }
